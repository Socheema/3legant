    ...
    <nav className="flex items-center space-x-2 text-lg w-full" aria-label="Breadcrumb">
      {items.map((item, index) => (
        <div key={index} className="flex items-center">
          {item.href ? (
            <Link href={item.href} className="text-[#343839] text-lg flex p-2 items-center gap-2">
               <span className='flex flex-1 items-center justify-center h-10 w-10 rounded-full bg-[#343839] text-[#F3F5F7]'>{index + 1}</span>
              {item.label}
            </Link>
          ) : (
            <span className=" flex text-lg items-center font-medium text-[#141718] flex-3 p-2 border-b gap-2 w-full min-w-[250px] max-w-[312px]" aria-current="page">
